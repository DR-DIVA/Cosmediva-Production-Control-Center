...
export function PlantDirectorAdvisory({
  ...
  return (
    <Card className="bg-gradient-to-br from-[#1E293B] via-[#0F172A] to-[#1E293B] text-white border border-[#D4AF37]/50 shadow-2xl rounded-2xl overflow-hidden relative mt-8 mb-8">
      {/* Luxury Gold & Sapphire Top Accent */}
      <div className="h-1.5 bg-gradient-to-r from-[#D4AF37] via-amber-300 to-[#D4AF37]"></div>

      <CardHeader className="p-5 md:p-6 border-b border-slate-800/80 bg-slate-900/50">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-gradient-to-br from-[#D4AF37]/30 to-amber-500/10 border border-[#D4AF37]/50 text-[#D4AF37] shadow-lg shrink-0">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <span className="text-[11px] font-bold text-amber-300 bg-amber-950/80 border border-amber-600/60 px-2.5 py-0.5 rounded-full shadow-inner flex items-center gap-1">
                  <Sparkles className="w-3 h-3 text-[#D4AF37]" /> คำแนะนำและข้อสั่งการระดับ ผอ.โรงงาน
                </span>
              </div>
          ...
          {/* KPI Alert Counter & Action Button */}
          <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto justify-end">
            <div className="flex items-center gap-2 bg-slate-800/80 p-1.5 rounded-xl border border-slate-700 text-xs">
              <span className={`px-2 py-0.5 rounded-lg font-bold flex items-center gap-1 ${
                criticalCount > 0 ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40' : 'bg-slate-700/50 text-slate-400'
              }`}>
                <Flame className="w-3 h-3" /> ด่วน {criticalCount} ประเด็น
              </span>
              <span className={`px-2 py-0.5 rounded-lg font-bold flex items-center gap-1 ${
                warningCount > 0 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40' : 'bg-slate-700/50 text-slate-400'
              }`}>
