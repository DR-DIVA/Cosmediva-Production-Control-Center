...
export async function POST(request: Request) {
  try {
    ...
    const itemRegex = /^[๓\s]*(\d+)[๓\s]+([A-Z0-9-]+)[๓\s]+([\s\S]{1,150}?)[๓\s]+([A-Z0-9]{2,6}|คลัง)?[๓\s]*([\d,.]+)[๓\s]*(KG|PCS|G|ML|L|KGS|EA|SET)/gim;
    ...
    if (items.length === 0) {
      ...
      const legacyTableRegex = /\s*๓\s*\d+\s*๓\s*([A-Z0-9-]+)\s+(.+?)\s*๓([A-Za-z0-9_-]+)๓\s*([\d,.]+)\s*(KG|kg|G|g|L|l|ML|ml|PCS|pcs|KGS|kgs|EA|ea|SET|set)/gi;
      let legacyMatch;
    ...
    if (items.length === 0) {
      const altItemRegex = /([A-Z0-9-]+)\s+([A-Za-z0-9\s/.-]+?)\s+([\d,.]+)\s*(KG|G|L|ML|PCS)/gi;
      let altMatch;
      while ((altMatch = altItemRegex.exec(text)) !== null) {
        if (altMatch[1].startsWith('R') || altMatch[1].length > 3) {
          items.push({
            rm_code: altMatch[1],
