            ...
            if (Array.isArray(histories)) {
              histories.forEach(h => {
                ...
                if (hDateStr === todayStr) {
                  historyItems.push({
                    taskId: task.id,
                    lotNo: (task as any).production_lots?.lot_no,
                    sku: (task as any).production_lots?.products?.sku,
                    tankNum,
                    action: h.status,
                    user: h.user,
      ...
      <div className="p-6 bg-[#F8F6F0] border-b shadow-inner">
        ...
        <div className="grid grid-cols-5 md:grid-cols-10 lg:grid-cols-12 gap-6">
          {tanks.map((t) => {
            ...
            const tankStatus = details[t]?.status || details[t] || (task.status === 'DONE' ? 'DONE' : 'WAITING')
