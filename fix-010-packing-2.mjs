...
async function checkData() {
  ...
  if (res.rows.length > 0) {
    ...
    const processRes = await pgClient.query("SELECT process_name FROM processes WHERE id = '" + res.rows[0].process_id + "'");
    if (processRes.rows[0].process_name !== 'บรรจุ') {
       console.log('Not packing process!');
       return;
    }
